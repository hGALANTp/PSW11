...
def cadastrar_empresa(request):
    if not request.user.is_authenticated:
        messages.add_message(request, constants.ERROR, "Efetuar o login")
    ...
    if request.method == "GET":
        ...
        try:
            empresa = Empresas(
                user=request.user, #aqui variavel recebe o usuario logado
                nome=nome,
                cnpj=cnpj,
                site=site,
...
def add_doc(request, id):
    empresa = Empresas.objects.get(id=id)
    titulo = request.POST.get('titulo') #request.POST.get server pra atribuir a variavel os valores inputados pelo usuarios na pagina.
    arquivo = request.FILES.get('arquivo') #request.FILE.get server pra atribuir a variavel os arquivos inputados pelo usuarios na pagina
    extensao = arquivo.name.split('.') #separa o arquivo no ponto, ex: teste.pdf fica [teste], [pdf]

    if empresa.user != request.user:
        messages.add_message(request, constants.ERROR, "Essa empresa não é sua")
        return redirect('/empresarios/empresa/')

    if extensao[1] != 'pdf': #aqui ele verifica se é pdf, na 2 posicao do dicionario.
